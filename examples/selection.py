...
"""This script demonstrates a few different ways you can perform selection in PMI.
...
# Create a molecule. This sets up "TempResidues" for all elements in the sequence.
mol = st.create_molecule("Rpb4",seqs["1WCM:D"],chain_id="D")

# No structure has been built yet - you have to call 'Molecule.add_representation'
...
# If you add a PDB, this returns the set of TempResidues that were in the PDB file
...
mol.add_representation(myres3,
                       resolutions=[1]) #creates beads with size 1 (non-atomic can only have one beadsize)
