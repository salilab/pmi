...
class CrossLinkingMassSpectrometryRestraint(IMP.pmi.restraints.RestraintBase):
    ...
    def __init__(self, root_hier, database=None, length=10.0, resolution=None,
        ...
        @param length maximal cross-linker length (including the residue
               sidechains)
        ...
        @param filelabel automatically generated file containing
        ...
        exdb = open("excluded." + filelabel + ".xl.db", "w")  # noqa: F841
...
class AtomicCrossLinkMSRestraint(IMP.pmi.restraints.RestraintBase):
    ...
    The "atomic" aspect is that it models the particle uncertainty with
    a Gaussian. The noise in the data and the structural uncertainty of
    cross-linked amino-acids is inferred using Bayes' theory of probability
    ...
    def __init__(self, root_hier, xldb, atom_type="NZ", length=10.0,
        ...
        @note Will return an error if the data+extra_sel don't specify
              two particles per XL pair.
        ...
        @param slope      Linear term to add to the restraint function to
               help when far away
        @param nstates    The number of states to model. Defaults to the
               number of states in root.
        ...
        @param one_psi    Use a single psi for all restraints (if False,
               creates one per XL)
        @param filelabel automatically generated file containing
                missing/included/excluded
                cross-links will be labeled using this text
        ...
        if filelabel is not None:
            indb = open("included." + filelabel + ".xl.db", "w")  # noqa: F841
            exdb = open("excluded." + filelabel + ".xl.db", "w")  # noqa: F841
            midb = open("missing." + filelabel + ".xl.db", "w")
    ...
    def plot_violations(self, out_prefix, max_prob_for_violation=0.1,
        ...
        @param max_prob_for_violation It's a violation if the probability
               is below this
        @param min_dist_for_violation It's a violation if the min dist
               is above this
...
class CysteineCrossLinkRestraint(object):
    def __init__(self, root_hier, filename, cbeta=False,
        ...
        # for that particular residue pair, eg, "Epsilon-Intra-Solvent", or
...
class DisulfideCrossLinkRestraint(object):
    def __init__(self, representation_or_hier,
        ...
        dr = IMP.isd.CrossLinkMSRestraint(self.m, length, slope)
