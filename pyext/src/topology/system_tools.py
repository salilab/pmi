...
def get_structure(model, pdb_fn, chain_id, res_range=None, offset=0,
    ...
    @param res_range Add only a specific set of residues.
           ...
           residue index
           The ending residue can be "END", that will take everything
...
def build_necklace(model, residues, resolution, input_coord=None):
    ...
    out_hiers = []
    for chunk in list(IMP.pmi.tools.list_chunks_iterator(residues,
                                                         resolution)):
...
def build_representation(parent, rep, coord_finder, rephandler):
    ...
    if rep.density_residues_per_component:
        ...
        density_frag = IMP.atom.Fragment.setup_particle(
            IMP.Particle(model), res_nums)
        density_frag.get_particle().set_name(
            "Densities %i" % rep.density_residues_per_component)
        density_ps = []

        if os.path.exists(rep.density_prefix + '.txt') \
                and not rep.density_force_compute:
            IMP.isd.gmm_tools.decorate_gmm_from_text(
                rep.density_prefix + '.txt', density_ps, model)
        if (len(density_ps) != num_components
                or not os.path.exists(rep.density_prefix + '.txt')
    ...
    for nr, r in enumerate(rsort):
        if (r.get_index() != prev_idx+1
                or r.get_has_structure() != prev_structure or force_break):
        ...
        for resolution in rep.bead_resolutions:
            ...
            if frag_res[0].get_has_structure():
                ...
                input_coord = coord_finder.find_nearest_coord(
                    min(r.get_index() for r in frag_res))
            ...
            # if volumetric, collect resolutions from different
            # segments together
            if single_node:
            ...
            if rep.setup_particles_as_densities:
                ...
                this_resolution.set_name(
                    this_resolution.get_name() + ' Densities %i' % resolution)
                this_representation.add_representation(this_resolution,
                                                       IMP.atom.DENSITIES,
                                                       resolution)
