...
class MembraneRestraintPrototype(IMP.Restraint):
    ...
    def score_inside(self,z):
        ...
        return -self.math.log(prob1*self.max_float)-self.math.log(prob2*self.max_float)+2*self.log_max_float
...
class MembraneRestraint(IMP.test.TestCase):
	def test_inside(self):
		...
		r2=IMP.pmi.MembraneRestraint(m,z_center.get_particle_index(), 30.0, 3.0, 0.0000000001, 0.02)
		...
		for z_c in range(-500,500,100):
			...
			for z in range(-500,500,10):
				...
				self.assertAlmostEqual(r.unprotected_evaluate(None),r2.unprotected_evaluate(None))
